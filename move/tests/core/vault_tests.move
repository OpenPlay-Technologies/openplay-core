...
public fun process_fees_ok() {
    ...
    let mut scenario = begin(addr);
    {
        ...
        // Process fees (both)
        vault.process_fees(10, 20);
        assert!(vault.play_balance() == 70);
        assert!(vault.collected_owner_fees() == 10);
        assert!(vault.collected_protocol_fees() == 20);

        // Process fees (just owner)
        vault.process_fees(10, 0);
        assert!(vault.play_balance() == 60);
        assert!(vault.collected_owner_fees() == 20);
        assert!(vault.collected_protocol_fees() == 20);

        // Process fees (just protocol)
        vault.process_fees(0, 10);
        assert!(vault.play_balance() == 50);
        assert!(vault.collected_owner_fees() == 20);
