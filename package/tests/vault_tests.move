...
#[test, expected_failure(abort_code = vault::EInsufficientFunds)]
public fun process_game_fees_fail() {
    let addr = @0xA;
    let game_id = object::id_from_address(@0xB);
    ...
    let mut scenario = begin(addr);
    {
        ...
        // Process fees
        vault.process_game_fee(game_id, 101);
        destroy(vault);
        abort 0
    }
}
...
public fun end_of_day_ok() {
    ...
    let mut scenario = begin(addr);
    {
        ...
        vault.process_game_fee(game_id, 10);
        ...
        assert!(vault.collected_game_fees(game_id) == 10);
        ...
        assert!(vault.collected_game_fees(game_id) == 20);
